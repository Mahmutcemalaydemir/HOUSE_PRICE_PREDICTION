...
# Bağımlı değişkenin histogram grafiğinin incelenmesi
...
# Kolonlardaki boşlukların No ile doldurulması
...
binary_cols = [col for col in df.columns if df[col].dtypes == "0" and len(df[col].unique()) == 2]
...
##############################
# Hiperparametre optimizasyonu gerçekleştiriniz
##############################

lgbn_model = LGBMRegressor(random_state=46)

rmse = np.mean(np.sqrt(-cross_val_score(lgbn_model, X, y, cv=5, scoring="neg_mean_squared_error")))

lgbn_params = {"learning_rate": [0.01, 0.1],
               "n_estimators": [500, 1500]}

lgbn_gs_best = GridSearchCV(lgbn_model,
                            lgbn_params,
                            cv=3,
                            n_jobs=-1,
                            verbose=True).fit(X_train, y_train)

final_model = lgbn_model.set_params(**lgbn_gs_best.best_params_).fit(X, y)
